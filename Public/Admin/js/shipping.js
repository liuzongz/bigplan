...
//		curTransType = $(this).next().next().attr('name').substring(6,8);
	...
	/*	首费离开校验*/
	$('.form.shipping').on('blur','input[data-field="postage"]',function (){
	...
	/*	续费离开校验*/
	$('.form.shipping').on('blur','input[data-field="postageplus"]',function (){
		var oNum = new Number($(this).val());
		oNum = oNum.toFixed(2);
		if (oNum > 999.99) oNum = 999.99;
		if (oNum=='NaN') oNum = '0.00';
	...
	/*	续件离开校验*/
	$('.form.shipping').on('blur','input[data-field="plus"]',function (){
		if ($(this).val() != ''){
			$(this).val($(this).toInt());
		}else{
			$(this).val(1);
	...
	/*	首件离开校验*/
	$('.form.shipping').on('blur','input[data-field="start"]',function (){
		if ($(this).val() != ''){
			$(this).val($(this).toInt());
		}else{
			$(this).val(1);
...
//        	obj_parent = $(this).parent().parent().parent();
	...
	$('.trans-line').on('click','.J_BatchSet',function(){
		if($('.tbl-except').find('input:checked').size()==0){
            Admin.info('请选择要批量设置的地区', 3); return false;
...
//		curTrans = $(this).parent().parent();
	...
	$('.trans-line').on('click','.J_BatchDel',function(){
		if($(this).parent().parent().find('.tbl-except').find('input:checked').size()==0){
            Admin.info('请选择要批量处理的地区', 3); return false;
