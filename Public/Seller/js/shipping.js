...
$(function(){
	...
	TransTpl += "默认运费：\n";
	TransTpl += "<input class=\"w30 mr5 text\" type=\"text\" aria-label=\"默认运费件数\" autocomplete=\"off\" data-field=\"start\" value=\"1\" name=\"shipping[0][0]\">\n";
	TransTpl += "件内，\n";
	TransTpl += "<input class=\"w60 text\" type=\"text\" aria-label=\"默认运费价格\" autocomplete=\"off\" value=\"\" name=\"shipping[0][1]\" data-field=\"postage\">";
	TransTpl += "<em class=\"add-on\">";
	TransTpl += "<i class=\"icon-renminbi\"><\/i>";
	TransTpl += "<\/em>， 每增加\n";
	TransTpl += "<input class=\"w30 ml5 mr5 text\" type=\"text\" aria-label=\"每加件\" autocomplete=\"off\" value=\"1\" data-field=\"plus\" name=\"shipping[0][2]\">\n";
	TransTpl += "件， 增加运费\n";
	TransTpl += "<input class=\"w50 ml5 text\" type=\"text\" aria-label=\"加件运费\" autocomplete=\"off\" value=\"\" data-field=\"postageplus\" name=\"shipping[0][3]\">";
    TransTpl += "<input type=\"hidden\" name=\"shipping[0][4]\" value=\"1^中国\">";
	TransTpl += "<em class=\"add-on\">";
	TransTpl += "<i class=\"icon-renminbi\"><\/i>";
	TransTpl += "<\/em>\n";
...
//	RuleCell += "<a class=\"ncu-btn2\" title=\"编辑运送区域\" area-haspopup=\"true\" area-controls=\"J_DialogArea\" entype=\'J_EditArea\' data-acc=\"event:enter\" href=\"JavaScript:void(0);\">编辑<\/a>\n";
	...
	RuleCell += "<input type=\"hidden\" value=\"\" name=\"shipping[CurNum][4]\" data-name=\"areas[TRANSTYPE][CurNum]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w30\" type=\"text\" aria-label=\"首件\" autocomplete=\"off\" value=\"1\" data-field=\"start\" name=\"shipping[CurNum][0]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"首费\" autocomplete=\"off\" value=\"\" data-field=\"postage\" name=\"shipping[CurNum][1]\"><em class=\"add-on\">\n";
	RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
	RuleCell += "<\/em>\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w30\" type=\"text\" aria-label=\"续件\" autocomplete=\"off\" value=\"1\" data-field=\"plus\" name=\"shipping[CurNum][2]\">\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"续费\" autocomplete=\"off\" value=\"\" data-field=\"postageplus\" name=\"shipping[CurNum][3]\"><em class=\"add-on\">\n";
	RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
	RuleCell += "<\/em>\n";
	RuleCell += "<\/td>\n";
...
//		SelectArea = new Array();
		...
		//返回选择的值到隐藏域
		$('input[data-name="areas['+curTransType+']['+curIndex.substring(1)+']"]').val(CityValue+'^'+CityText);
		//关闭弹出层与遮罩层
	    $("#dialog_areas").css('display','none');
	    $('.ks-ext-mask').css('display','none');
	    //清空check_num显示的数量
...
//		curTransType = $(this).next().next().attr('name').substring(6,8);
		...
		var expAreas = $('input[data-name="areas['+curTransType+']['+curIndex.substring(1)+']"]').val();
		...
		$(objCurlArea).parent().parent().find('.area-group').each(function(){
			if ($(this).next().attr('data-name') != 'areas['+curTransType+']['+curIndex.substring(1)+']'){
	...
	/*	首费离开校验*/
	$('.ncsc-form-default').on('blur','input[data-field="postage"]',function (){
	...
	/*	续费离开校验*/
	$('.ncsc-form-default').on('blur','input[data-field="postageplus"]',function (){
		var oNum = new Number($(this).val());
		oNum = oNum.toFixed(2);
		if (oNum > 999.99) oNum = 999.99;
		if (oNum=='NaN') oNum = '0.00';
	...
	/*	续件离开校验*/
	$('.ncsc-form-default').on('blur','input[data-field="plus"]',function (){
		if ($(this).val() != ''){
			$(this).val($(this).toInt());
		}else{
			$(this).val(1);
	...
	/*	首件离开校验*/
	$('.ncsc-form-default').on('blur','input[data-field="start"]',function (){
		if ($(this).val() != ''){
			$(this).val($(this).toInt());
		}else{
			$(this).val(1);
	...
	$('.trans-line').on('click','.J_DeleteRule',function (){
        ...
        Seller.confirmDialog('确认删除吗？', function(){
...
//        	obj_parent = $(this).parent().parent().parent();
	...
	$('.trans-line').on('click','.J_BatchSet',function(){
		if($('.tbl-except').find('input:checked').size()==0){
            Seller.infoDialog('请选择要批量设置的地区', 3); return false;
...
//		curTrans = $(this).parent().parent();
	...
	$('.trans-line').on('click','.J_BatchDel',function(){
		if($(this).parent().parent().find('.tbl-except').find('input:checked').size()==0){
            Seller.infoDialog('请选择要批量处理的地区', 3); return false;
