...
var Jiniu = {
    ajax : function(url, params, callback, transferMode, responseType, asyn){
        var data = {};
        data.type = typeof(transferMode) === "string" && transferMode.toUpperCase() === "GET" ? "GET" : "POST";
        if (transferMode === "GET")  {
            var d = new Date();
        ...
        data.dataType = responseType.toLowerCase();
        data.asyn = (asyn == false ? false : true);
        data.success = function (data) {
            if (typeof(callback) === "function") { callback(data);}
            return data;
        };
        data.error = function (ex) {return ex;};
        console.log(params);
        if (typeof(params) == 'object') {
            data.processData = false;         // 告诉jQuery不要去处理发送的数据
            data.contentType = false;        // 告诉jQuery不要去设置Content-Type请求头
            data.xhrFields = {withCredentials: true};
            data.crossDomain = true;
            console.log('is_object');
        } else {
            console.log('not is_object');
                }
        return $.ajax(data);
            },
    /**
    ...
    convertBase64UrlToBlob: function(urlData){
        //去掉url的头，并转换为byte
        if(urlData =='' || urlData == 'undefined' || !urlData) return;
    ...
    showPreview:function (source){
        ...
        if(!/image\/\w+/.test(file.type)){
            result = this.result_ajax(300,"上传的图片必须是图片类型");
        } else {
            if (window.FileReader) {
                ...
                fr.onloadend = function(e) {
                    s = 1;
                    result = this.result_ajax(200,'转换成功！',{'data':e.target.result});
                    alert('ssssss');
                    /*console.log(e);
                    var img = $(source).next('img'),
                        error = "alert('图片上传错误,请重新选择图片');$(this).attr('src',null).css('width',0);";
                    img.attr({'src':e.target.result,onerror:error}).css('width','100%');
                    $(source).prevAll('.flex').addClass('none');
                    $(source).nextAll('.shangchuan').removeClass('none');*/
                };
                ...
                result = this.result_ajax(301,"浏览器不支持上传文件");
            }
        }
    },
    upimage : function(source, url, callback, enzip, maxsize){
        ...
        if(!/image\/\w+/.test(file.type)){
            ...
            if (window.FileReader) {
                ...
                fr.onloadend = function(e) {
                    ...
                    var images =   new Image();//创建一个图片
                    var result = '';
                    images.src = e.target.result;
                    if (enzip && (images.width > maxsize || images.height > maxsize)) {
                        result = self.unzip_image(images, maxsize);
                    } else {
                        result = e.target.result;
                    }
                    var bytes = self.convertBase64UrlToBlob(result);
                    formData.append("file_" + file, bytes,  'file_' + file + '.jpg');
                    return Jiniu.ajax(url,formData,callback,'POST','json',false);
                };
                fr.readAsDataURL(file);
            } else {
                alert("浏览器不支持上传文件");
    ...
    unzip_image : function(Img, maxsize, onCompress){
        onCompress = onCompress || function(ex){return ex};
        var canvas = document.createElement('canvas'); //new canvas;
        //agoimg.after("<canvas  class='myCanvas' style='display: none' ></canvas>");
        //var Cnv = agoimg.next('canvas')[0];
        var Cntx = canvas.getContext('2d');//获取2d编辑容器

        if (Img.width > Img.height) {
            canvas.width = maxsize ;
            canvas.height = (maxsize / Img.width) * Img.height;
...
WebApp.Widget = function(){
    ...
    that.scroll_load = function($obj, $cfg){
        ...
        var load_type = $cfg['load_type'] == 'scroll' || !$cfg['load_type'] ? 'scroll' : 'click';
        var page = 1,is_Querystring = function(url){
            ...
            $sObj.unbind('scroll');
                ...
                show_status.unbind('click');
            }
            if(page >1)  show_status.html('<span><img src="/Public/Static/images/loading1.gif" /></span><span>加载中....</span>');
                ...
                if (ex.error == 200) {
                    ...
                    if (page > ex.data.pagecount) {
                        ...
                        show_status.text('上拉加载更多！');
                            ...
                            show_status.click(get_prev).text('点击加载更多！');
                    }
                    }
                } else {
...
var hkhp = {
    ...
    Hremn:function(rs) {        //像素转换rem
        return (rs / ($(window).width() / 10));
    }
};
...
var Jiniu1 = {
    loading:function(text){
        var laoding  = '<div class="wx-loading"><em>'+text+'</em></div>';
        $('body').append(laoding);
    },
    closeloading:function(){
        $('div.wx-loading').remove();
    },
    ...
    clone:function(obj,show,str,liClass,maxlength,callback){
    ...
    textChange:function(text,show){  //输入框，显示容器
        $(text).bind('change',function(){
            ...
            var text2 = text1.split('\r').join('&nbsp;');
            $(this).next(show).html(text2);
        });
    },
    ...
    textNumber:function(obj,length,maxLength){
        obj.each(function(){

        })
    },
    autoHeight : function(obj,maxLength){
        ...
        obj.each(function(){
            ...
            $(this).on('keyup click', function(){
                autoHeight(this);
                var myLen = Jiniu1.getBytesLength($(this).val());
                var index = $(this).prev('span.text-limit');
                index.html(myLen+'/'+maxLength);
                if(myLen>maxLength)  index.css('color','red');
                else  index.css('color','');
            });
        });
    }
};
