...
class Activity extends Object{
     ...
     * @return mixed|null
     */
    public function getDataOfAttend($user_id){
        if(!$this->isExist() && $user_id) return null;
        if(!($this->project instanceof Project)) return null;
        $where['a.action_id'] = $this->project->projectId;
        $where['a.user_id'] = $user_id;
        $where['a.parent_id'] = 0;
        return M()->table('__USERS_ACTIVITY__ a')->join(' LEFT JOIN __FAVOURABLE_ORDER__ b ON a.user_id=b.user_id AND a.id=b.attend_id AND b.pay_status='.PS_PAYED)->where($where)->find();
    }
    ...
    public function getDataOfUser(){
        if(!$this->isExist()) return null;
        if(!($this->project instanceof Project)) return null;
        $where['action_id'] = $this->project->projectId;
        return M('users_activity')->where($where)->select();
    }
     ...
     * @return bool|null
     */
    public function attend($user_id){
        if(!$this->isExist()) return null;
        return $this->_favourable->attend($this, $user_id);
    }
     ...
     * @return array
     */
    public function getActive($act_id){
        if(!$this->isExist()) return null;
        $where['act_id'] = $act_id;
        return M('favourable_user_activity')->where($where)->select();
    }
     ...
     * @return array
     */
    public function rebate($attend_id){
        if(!$this->isExist()) return null;
        return $this->_favourable->rebate($this,$attend_id);
    }
