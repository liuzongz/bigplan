...
abstract class BaseController extends Controller {
    ...
    protected function _initialize(){
        $this->time = time();
        //$this->login_info = session('login_info_');
        ...
        $this->jump_url     = I('request.jump_url', '', 'AesDeCrypt');//设置回跳url
        $this->back_module  = I('request.back_module', '', 'AesDeCrypt');
        //$this->get_cur_url();
     ...
     * @return bool
     */
    protected function void_user($store_id = '', $url = '', $sess_time = 3600){
        $time = time();
        $this->login_info =  session('login_info_' . $store_id);
        if ($this->is_login($store_id)){
            ...
            session('login_info_' . $store_id, null);
            session('wx_token' . $store_id, null);
            //if($this->is_weixin){
                $url = get_server('PASSPORT_SERVER', '/User/login',
                    [
                        'back_act'  =>   AesEnCrypt($this->cur_url),
                        C('store_token_name') => $this->store_id
                    ], 1);
            //}
     ...
     * @return bool
     */
    protected function is_login($store_id = '', $login_sess = array()){
    ...
    protected function is_login1($login_sess = array()){
        $login_sess = !empty($login_sess) ? $login_sess : session('login_info');
        if ($login_sess and $login_sess['user_id'] > 0) {
            $user_info = M('users')
                ->alias('u')
