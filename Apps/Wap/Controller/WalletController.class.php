...
use Wap\Model;
class WalletController extends WapController {
    ...
    public function index(){
        ...
        $this->assign('user_head', array('title'=>'我的账户'));
    ...
    public function dynamic(){
        $user = new Model\UsersModel();
        $res = $user->get_user_account($this->user_id, 10);
        $time = time();
        $prev_day = strtotime(date('Y-m-d', $time - 60 * 60 * 24));
        $today = strtotime(date('Y-m-d', $time));
        foreach ($res['list'] as &$v) {
        ...
        $this->assign('user_head', array('title'=>'我的钱包'));
        $this->assign('tkd', ['title'=>'我的账户']);
        $this->assign('user_info', $this->user_info);
        $this->display();
    }
        ...
        if (!$cash_account) {
            ...
            if (IS_POST) {
                ...
                if (!$this->check_form_token()) {
                    $this->error('重复提交失败，请在页面进行提交！');
                } elseif ($cash_num < $min_cash) {
                    $this->error('提现错误，提现金额不得小于' . $min_cash . '！');
                } elseif ($cash_id <= 0 or $cash_id != $cash_account['id']) {
                    $this->error('提现错误，提现帐号不存在！');
                ...
                $this->assign('user_info', $this->user_info);
                $this->assign('user_head', array('title'=>'钱包提现','backUrl'=>get_server('PASSPORT_SERVER','/user/index', ['store_token'=>$this->store_token], 1),'backText'=>'会员首页'));
                $this->assign('tkd', ['title'=>'钱包提现']);
                $this->set_form_token();
                $this->display();
            }
