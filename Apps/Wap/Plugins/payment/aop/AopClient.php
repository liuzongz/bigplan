<?php
namespace Wap\Plugins\payment\alipay;
use Wap\Controller;
require_once 'AopEncrypt.php';
class AopClient extends Controller\WapController {
	//应用ID
	public $appId;
	//私钥文件路径
	...
	protected function curl($url, $postFields = null) {
		...
		if (curl_errno($ch)) {
			...
			throw new \Think\Exception('Alipay:' . curl_error($ch), 0);
		} else {
			$httpStatusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
			if (200 !== $httpStatusCode) {
				throw new \Think\Exception( 'Alipay:' . $reponse, $httpStatusCode);
			}
		}
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (strcasecmp($this->fileCharset, $this->postCharset)) {
			...
			throw new \Think\Exception('Alipay:' . "文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
		}
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			if ($this->checkEmpty($apiParams['biz_content'])) {
				...
				throw new \Think\Exception("'Alipay:' api request Fail! The reason : encrypt request is not supperted!");
			}

			if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

				throw new \Think\Exception("'Alipay:' encryptType and encryptKey must not null! ");
			}

			if ("AES" != $this->encryptType) {

				throw new \Think\Exception("'Alipay:'加密类型只支持AES");
			}
...
/*print_r($sysParams);
print_r($apiParams);
        echo $requestUrl ;exit('end');*/
        $pay_info = array(
            'url'  => $requestUrl,
            'biz_content'   =>  htmlspecialchars($apiParams['biz_content'])
        );
		...
		//发起HTTP请求
		try {
            $resp = $this->curlGet($requestUrl,'post',$apiParams);
			//$resp = $this->curl($requestUrl, $apiParams);
		} catch (\Think\Exception $e) {

            //\Think\Log::record('Alipay:' . $sysParams["method"] . '' .  $requestUrl . '' .  "HTTP_ERROR_"  . '' .  $e->getCode() . '' .  $e->getMessage());
		...
		if (false === $respWellFormed) {
            //\Think\Log::record($sysParams["method"], $requestUrl, "HTTP_RESPONSE_NOT_WELL_FORMED", $resp);
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		if (!$this->checkEmpty($this->alipayPublicKey)) {
			if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty($signData->signSourceData)) {
				...
				throw new \Think\Exception("'Alipay:' check sign Fail! The reason : signData is Empty");
			}
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						if (!$checkResult) {
							throw new \Think\Exception("'Alipay:'check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
						}
