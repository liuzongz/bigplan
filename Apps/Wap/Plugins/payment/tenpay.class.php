...
$payment_lang =  '../languages/' .$GLOBALS['_CFG']['lang']. '/payment/tenpay.php';
...
class tenpay
{
     ...
     */
    function get_code($order, $payment, $is_link = false)
    {
        $cmd_no = '1';
        ...
        /* 编码标准 */
        if (!defined('MJJ_CHARSET') || MJJ_CHARSET == 'utf-8')
        {
        ...
        $parameter = array(
            ...
            'spbill_create_ip'    => $spbill_create_ip                 //财付通分配的商户号

        );
		if($is_link){
        	$button  = '<form style="text-align:center;" action="https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi" style="margin:0px;padding:0px" name="tenpay_go">';
			foreach ($parameter AS $key => $val){
				$button  .= "<input type='hidden' name='$key' value='$val' />";
			}
			//$button  .= '<input type="submit" class="btn_now_pay" value="" style="display:none;" />';
			$button  .= '</form><br /><script type="text/javascript">tenpay_go.submit();</script>';	
		}else{
			$button  = '<form style="text-align:center;" action="https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi" target="_blank" style="margin:0px;padding:0px" name="tenpay_go">';
			foreach ($parameter AS $key => $val){
				$button  .= "<input type='hidden' name='$key' value='$val' />";
			}
			$button  .= '<input type="submit" class="btn_now_pay" value="" />';
			$button  .= '</form><br />';	
		}
        return $button;
    }
