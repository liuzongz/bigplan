...
class integrate
{
    ...
    {
		$user_id = $this->check_user($username, $password);
        if ($user_id > 0)
        {
            if ($this->need_sync)
            {
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
        {
            ...
            if ($col)
            {
                ...
                $GLOBALS['db']->query($sql);
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_validated') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                $GLOBALS['db']->query($sql);
                /* 删除用户订单 */
                ...
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_bonus') . " WHERE " . db_create_in($col, 'user_id'); //删除用户优惠券
                $GLOBALS['db']->query($sql);
    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            ...
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE (" . $this->field_name . "='" . $post_username . "' or " .$this->field_email . "='" . $post_username . "') AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "'";
    ...
    function set_cookie($username='', $remember= null )
    {
        ...
        elseif ($remember)
        {
            ...
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' or email='$username' LIMIT 1";
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' or email='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
