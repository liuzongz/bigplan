...
class UsersModel extends BaseModel{
    ...
    public function get_myteam2($user_id, $store_token, $max = 5){
        if(empty($user_id)) return [];
        if (!is_array($user_id)) {
            $user_id = array($user_id);
        }
        $in = $this->db_create_in($user_id,'ul.parent_id');
        $in .= 'and ' . $this->db_create_in($store_token,'ul.store_id');
        $sql = $this->alias('u')
            ->field('u.user_id')
            ->join('LEFT JOIN __USER_RELEVANCE__ ul ON u.user_id=ul.user_id')
            ->where($in)->buildSql();// 查询满足要求的总记录数

        $map = 'ul.user_id IN' . $sql . 'and ul.store_id=' . $store_token;
        ...
        $result = $this->alias('u')
            ...
            ->join('LEFT JOIN __ORDER_INFO__ oi ON oi.user_id=u.user_id and oi.pay_status=2')
...
//            $order_count += $v['order_count'] ;
     ...
     * @return bool|string
     */
    public function user_qr_add($user_id, $md5, $address, $expire_seconds=604800) {
        $data = array(
            'md5'           =>  $md5,
            'user_id'       =>  (int)$user_id,
            'img_address'   =>  $address,
     ...
     * @return mixed
     */
    public function user_qr_get($user_id,$store_id){
    ...
    public function add_user($classData, $uid = 0) {
        ...
        if ($this->is_email($classData['user'])) {
        ...
        //$data['mobile_phone']   = '';
        $data['credit_line']    = 0;
        $data['rank_points']    = $this->_CFG['register_points'];
    ...
    public function get_user_agency($user_id){
        $where = " uid={$user_id} and state=" . YJ_UNAUDIT;
